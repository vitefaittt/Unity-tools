...
public static class MoveArrayElementEditor
{
    ...
    static void OnPropertyContextMenu(GenericMenu menu, SerializedProperty property)
    {
        ...
        GUIContent moveUp = new GUIContent("Move Array Element Up");
        if (index > 0)
            menu.AddItem(moveUp, false, () => MoveDown(serializedTargetObject, serializedArray, index));
        else
            menu.AddDisabledItem(moveUp);

        GUIContent moveDown = new GUIContent("Move Array Element Down");
        if (index < arrayLength - 1)
            menu.AddItem(moveDown, false, () => MoveUp(serializedTargetObject, serializedArray, index));
        else
